# =============================================================================
# Assign-MDVMPermissions.gov.ps1
# =============================================================================
# PURPOSE:
#   GCC High / DoD variant of Assign-MDVMPermissions.ps1.
#   Uses the Azure US Government identity plane (login.microsoftonline.us,
#   graph.microsoft.us, AzureUSGovernment) and the api-gov Defender endpoint.
#   NOTE: GCC (Moderate) customers must use the gcc/ folder instead.
...
#
# WHEN TO RUN:
#   After deploying MDETVM-LogicApp.gcc-high.json to an Azure US Government
#   (GCC High / DoD) tenant.
#   The Logic App, DCE/DCR, and custom table must all be deployed first.
#
# PREREQUISITES:
#   - Az PowerShell module: Install-Module Az -Scope CurrentUser
#   - Global Admin or Application Administrator role in the GCC High / DoD Entra tenant
#   - Owner or User Access Administrator on the resource group (for DCR role)
#   - Run from a machine able to reach login.microsoftonline.us / graph.microsoft.us
#
# CREDITS:
...
Write-Host "Connecting to Azure US Government (GCC High / DoD tenant: $TenantId)..." -ForegroundColor Cyan
Connect-AzAccount -TenantId $TenantId -Environment AzureUSGovernment

# GCC High / DoD Microsoft Graph endpoint
...
if ($null -eq $resource) {
    Write-Error "WindowsDefenderATP service principal not found. Verify Microsoft Defender for Endpoint is provisioned in this GCC tenant."
    exit 1
}
...
foreach ($permission in @("Vulnerability.Read.All")) {
    ...
    $response = (Invoke-AzRestMethod `
        -Method POST `
        -Uri ("$graphBaseUri/v1.0/servicePrincipals/" + $resource.Id + "/appRoleAssignedTo") `
