...
# =============================================================================
# PURPOSE:
#   GCC (Moderate) variant of Assign-MDVMPermissions.ps1.
#   GCC uses the COMMERCIAL identity plane (login.microsoftonline.com,
#   graph.microsoft.com, AzureCloud) - only the Defender API endpoint is gov
#   (api-gcc). So this script connects to Azure Commercial, NOT US Government.
...
#
# WHEN TO RUN:
#   After deploying MDETVM-LogicApp.gcc.json to an Azure Commercial (GCC) tenant.
...
#   - Global Admin or Application Administrator role in the GCC Entra tenant
#   - Owner or User Access Administrator on the resource group (for DCR role)
#   - Run from a machine able to reach login.microsoftonline.com / graph.microsoft.com
#
# CREDITS:
...
Write-Host "Connecting to Azure Commercial (GCC tenant: $TenantId)..." -ForegroundColor Cyan
Connect-AzAccount -TenantId $TenantId

# Commercial Microsoft Graph endpoint (GCC uses the commercial identity plane)
