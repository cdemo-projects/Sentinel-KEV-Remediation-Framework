...
|---|---|---|
| **Native TVM Comparison + Workbook** | Directly correlates native Defender XDR `DeviceTvm*` tables with the live CISA KEV catalog; no custom table or duplicate ingestion | Defender Vulnerability Management; Sentinel workspace connected to the Defender portal for the workbook |
...
| **Custom-table Sentinel Workbook** | Visual dashboard backed by retained `MDETVM_CL` snapshots | TVM Data Ingest |
| **KEV Remediation** | SOAR playbook: auto-remediate via WUfB (Windows KBs) or Intune (third-party) | Sentinel Analytics + Intune |

---

## Native Access vs. Custom Ingestion

Microsoft Sentinel workbooks in the Defender portal can use **Advanced hunting** as a data source and query Defender XDR tables directly. The native workbook uses that access path. The original ingestion pipeline remains available because copying MDVM findings into `MDETVM_CL` supports Sentinel capabilities that require the data in Log Analytics.

| Capability | Native Advanced Hunting path | Custom `MDETVM_CL` path |
| --- | --- | --- |
| Source | Live `DeviceTvmSoftwareVulnerabilities` and `DeviceTvmSoftwareVulnerabilitiesKB` state | MDVM REST API export copied into Log Analytics |
| Data movement | No duplicate ingestion by this project | Logic App → Data Collection Rule → `MDETVM_CL` |
...
│   ├── commercial/                  ← Azure Commercial templates + perms
│   ├── gcc/                         ← GCC (hybrid: commercial identity, gov Defender API)
│   ├── gcc-high/                    ← GCC High / DoD templates + perms
│   └── shared/                      ← Verify-MDVMTables.kql, Pipeline-Health-Alerts.json
│
...
│
├── sentinel-workbooks/              ← Native and custom-table dashboards
│   ├── MDETVM-KEV-Workbook.json
│   ├── MDETVM-KEV-Workbook-Native.json
│   ├── CISA-KEV-Native-Comparison.kql
│   └── KEV-Workbook-GalleryTemplate.json
│
└── kev-remediation/                 ← KEV auto-remediation — one folder per approach
    ├── incident-triggered/          ← Sentinel playbook: WUfB expedite + Win32 (commercial/gcc/gcc-high)
    ├── wufb-expedite/               ← Windows-KB expedite (label; logic in incident-triggered Path A)
    ├── win32-static/                ← Version-pinned Win32 app assignment + packaging
    ├── eac-autoupdate/              ← EAC auto-update — API-direct, no Sentinel (commercial/gcc/shared)
    ├── firstparty-ondevice-script/  ← Intune Remediation native-updater script (POC)
    ├── fallback-ticketing/          ← ServiceNow ticket + Intune task fallback (+ KEV-ServiceNow guide)
    └── _shared/                     ← Rings, audit table, mapping host, baseline, operator runbook
...
### Option 3 — Detection + Incidents (Alerting)

```powershell
# After Option 2, add the analytics rules
az deployment group create -g <rg> `
...
### Option 4 — Full Automation (Remediation)

```powershell
# After Option 3, deploy the remediation playbook
az deployment group create -g <rg> `
...
| **Lab-only third-party install path** ✅ *Resolved* | The legacy `Update-KEVRemediateThirdPartyPath.ps1` script made each device pull installers from sites like `github.com` and `7-zip.org` at runtime — fine for a lab tenant, not appropriate for prod or gov | **Resolved by the Win32 app assignment path.** See [`kev-remediation/commercial/KEV-Remediate-Win32-Snippet.json`](kev-remediation/commercial/KEV-Remediate-Win32-Snippet.json), [`kev-remediation/gcc/KEV-Remediate-Win32-Snippet.gcc.json`](kev-remediation/gcc/KEV-Remediate-Win32-Snippet.gcc.json), and [`kev-remediation/gcc-high/KEV-Remediate-Win32-Snippet.gcc-high.json`](kev-remediation/gcc-high/KEV-Remediate-Win32-Snippet.gcc-high.json) (drop-in Logic App scopes), [`kev-remediation/shared/Win32-App-Mapping.json`](kev-remediation/shared/Win32-App-Mapping.json) (CVE→app GUID lookup), [`kev-remediation/shared/Package-Win32App-Guide.md`](kev-remediation/shared/Package-Win32App-Guide.md) (admin packaging runbook), and the matching `Assign-KEVRemediatePermissions` script in each cloud folder (with `DeviceManagementApps.ReadWrite.All`). Legacy script archived to [`_archive-remediation-poc/`](_archive-remediation-poc/). |
