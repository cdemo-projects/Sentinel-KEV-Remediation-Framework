<#
.SYNOPSIS
    GCC High / DoD variant: assigns permissions to the KEV-Remediate Logic App managed identity.
...
.EXAMPLE
    .\Assign-KEVRemediatePermissions.gcc-high.ps1 `
...
Write-Host "`n[Done] All permissions assigned for '$LogicAppName' (GCC High / DoD)." -ForegroundColor Green
...
Write-Host "  Lock it down in Exchange Online Protection (GCC High / DoD) with an Application Access Policy:" -ForegroundColor Yellow
