<#
.SYNOPSIS
    GCC variant: assigns permissions to the KEV-Remediate Logic App managed identity.
...
.DESCRIPTION
    ...
    Differences from commercial:
      - Connect-AzAccount uses -Environment AzureCloud
      - Graph endpoint is https://graph.microsoft.com
...
.EXAMPLE
    .\Assign-KEVRemediatePermissions.gcc.ps1 `
...
# GCC endpoints (commercial identity plane, gov Defender API only)
$graphBaseUri = "https://graph.microsoft.com"

# --- Connect ---
Write-Host "[0/5] Connecting to Azure Commercial (GCC tenant) (Tenant: $TenantId)..." -ForegroundColor Cyan
Connect-AzAccount -TenantId $TenantId -Environment AzureCloud | Out-Null
...
# Phase 5 (production scale) additions:
#   Group.Read.All                          - read members of universal ring groups (Pilot/Early/Broad)
#   GroupMember.ReadWrite.All               - add/remove devices to per-app AppPatch groups at incident time
...
Write-Host "`n[Done] All permissions assigned for '$LogicAppName' (GCC)." -ForegroundColor Green
...
Write-Host "  Lock it down in Exchange Online Protection (GCC) with an Application Access Policy:" -ForegroundColor Yellow
