...
# Poll commit (Microsoft official sample uses 600 attempts x 10s = 100 min;
# we use a more reasonable 30 min total but extend if your tenant takes longer)
$committed = $false
for ($i = 0; $i -lt 180; $i++) {
    ...
    if ($i % 6 -eq 0) { Write-Host "      polling commit ... ${i}/180" -ForegroundColor DarkGray }
}
if (-not $committed) { throw "Timed out waiting for commitFileSuccess after 30 min. Manually delete the orphan app: $appId" }
