...
| `Intune-Windows-Pilot-Devices` | Assigned device group | First managed Windows devices, including `device01` after enrollment |
...
7. Enroll `device01` using Settings > Accounts > Access work or school > Enroll only in device management.
8. Add the enrolled `device01` device object to `Intune-Windows-Pilot-Devices`.
...
These findings came from the commercial tenant proof on `device01`:
