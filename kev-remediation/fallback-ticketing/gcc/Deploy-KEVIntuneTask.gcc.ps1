<#
.SYNOPSIS
    Deploys the KEV-IntuneTask-v1 Logic App (GCC) — Phase A: HTTP-triggered minimal PoC.
...
.DESCRIPTION
    ...
    from KEV-IntuneTask-v1.gcc.json.

    After this script:
      1. Run Assign-KEVRemediatePermissions.gcc.ps1 against THIS Logic App name to grant
...
$defPath = Join-Path $PSScriptRoot "KEV-IntuneTask-v1.gcc.json"
if (-not (Test-Path $defPath)) { throw "Workflow definition not found: $defPath" }
$definition = Get-Content $defPath -Raw | ConvertFrom-Json

Write-Host "[1/3] Acquiring management token (GCC / commercial ARM)..." -ForegroundColor Cyan
...
$laUri = "https://management.azure.com/subscriptions/$SubscriptionId/resourceGroups/$ResourceGroupName/providers/Microsoft.Logic/workflows/${LogicAppName}?api-version=2016-06-01"
...
$cbUri = "https://management.azure.com/subscriptions/$SubscriptionId/resourceGroups/$ResourceGroupName/providers/Microsoft.Logic/workflows/$LogicAppName/triggers/manual/listCallbackUrl?api-version=2016-06-01"
...
Write-Host "  .\Assign-KEVRemediatePermissions.gcc.ps1 -LogicAppName $LogicAppName -ResourceGroupName $ResourceGroupName -WorkspaceName <ws> -TenantId <tenant>" -ForegroundColor DarkGray
